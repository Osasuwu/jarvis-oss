...
# Load .env: check local first, then sibling telegram-mcp repo
_script_dir = os.path.dirname(os.path.abspath(__file__))
_repo_root = os.path.dirname(_script_dir)
for _env in [
    os.path.join(_script_dir, ".env"),
    os.path.join(_repo_root, "..", "telegram-mcp", ".env"),
]:
...
# ACL Configuration - Two-tier access control for chats
_RW_CHATS_ENV = os.getenv("TELEGRAM_RW_CHATS", "").strip()
_RO_CHATS_ENV = os.getenv("TELEGRAM_RO_CHATS", "").strip()

_rw_ids: set = set()
_rw_usernames: set = set()
_ro_ids: set = set()
_ro_usernames: set = set()
_ACL_ENABLED: bool = False


def _parse_acl_config():
    """Parse ACL environment variables and populate id/username sets."""
    global _ACL_ENABLED, _rw_ids, _rw_usernames, _ro_ids, _ro_usernames

    # Parse TELEGRAM_RW_CHATS
    if _RW_CHATS_ENV:
        for item in _RW_CHATS_ENV.split(","):
            item = item.strip()
            if not item:
                continue
            try:
                ...
                # Treat as username, normalize to lowercase without @
                username = item.lstrip("@").lower()
                _rw_usernames.add(username)

    # Parse TELEGRAM_RO_CHATS
    if _RO_CHATS_ENV:
        for item in _RO_CHATS_ENV.split(","):
            item = item.strip()
            if not item:
                continue
            try:
                ...
                # Treat as username, normalize to lowercase without @
                username = item.lstrip("@").lower()
                _ro_usernames.add(username)

    _ACL_ENABLED = bool(_rw_ids or _rw_usernames or _ro_ids or _ro_usernames)


def _matches_acl(identifier, id_set: set, username_set: set) -> bool:
    """Check if identifier matches any item in id_set or username_set.

    Args:
        identifier: int (chat_id), str (username or string ID), or object with .id/.username
        id_set: set of integer IDs to match against
        username_set: set of usernames to match against
    ...
    if isinstance(identifier, int):
        return identifier in id_set

    # Handle string IDs or usernames
    if isinstance(identifier, str):
        try:
            # Try to parse as integer
            chat_id = int(identifier)
            return chat_id in id_set
        except ValueError:
            # Treat as username, normalize
            username = identifier.lstrip("@").lower()
            return username in username_set

    # Handle entity objects with .id and .username attributes
    if hasattr(identifier, "id"):
        if identifier.id in id_set:
            return True

    if hasattr(identifier, "username") and identifier.username:
        username = identifier.username.lstrip("@").lower()
...
def _has_access(identifier) -> bool:
    """Check if identifier has read access (in RW or RO list)."""
    if not _ACL_ENABLED:
        return True

    if _matches_acl(identifier, _rw_ids, _rw_usernames):
        return True

    if _matches_acl(identifier, _ro_ids, _ro_usernames):
        return True

    return False


def _has_write_access(identifier) -> bool:
    """Check if identifier has write access (in RW list only)."""
    if not _ACL_ENABLED:
        return True

    return _matches_acl(identifier, _rw_ids, _rw_usernames)


def _acl_deny_msg(identifier, action: str) -> str:
    """Return error message for denied access."""
    return f"Access denied: chat {identifier} does not have {action} permission."


def _acl_readonly_msg(identifier) -> str:
    """Return error message for write operation on read-only chat."""
    return f"Access denied: chat {identifier} is read-only."
...
async def get_chats(page: int = 1, page_size: int = 20) -> str:
    ...
    try:
        ...
        # Filter by ACL if enabled
        if _ACL_ENABLED:
            dialogs = [d for d in dialogs if _has_access(d.entity)]
...
async def get_messages(chat_id: Union[int, str], page: int = 1, page_size: int = 20) -> str:
    ...
    """
    try:
        if not _has_access(chat_id):
            return _acl_deny_msg(chat_id, "read")
...
async def send_message(
    ...
    """
    try:
        if not _has_write_access(chat_id):
            if _has_access(chat_id):
                return _acl_readonly_msg(chat_id)
            return _acl_deny_msg(chat_id, "write")
        entity = await resolve_entity(chat_id)
...
async def list_chats(
    ...
    try:
        ...
        for dialog in dialogs:
            ...
            # Filter by ACL if enabled
            if _ACL_ENABLED and not _has_access(entity):
                continue
...
async def forward_message(
    ...
    """
    try:
        # Check read access on source chat
        if not _has_access(from_chat_id):
            return _acl_deny_msg(from_chat_id, "read")
        # Check write access on destination chat
        if not _has_write_access(to_chat_id):
            if _has_access(to_chat_id):
                return _acl_readonly_msg(to_chat_id)
            return _acl_deny_msg(to_chat_id, "write")
...
async def _main() -> None:
    try:
        # Start MCP server immediately — Telegram connects lazily on first tool call
        # via ensure_connected(). resolve_entity() handles cache warming on demand.
        print("Starting MCP server (Telegram connects on first use)...", file=sys.stderr)
