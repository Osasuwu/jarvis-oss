...
NEG_PATTERNS = [
    ...
    r"\bпочему\s+ты\b",
    r"\bопять\b",
    r"\bя\s+же\s+(говорил|сказал|просил)\b",
    r"\bты\s+не\s+поня",
    r"\bперестань\b",
    r"\bхватит\b",
    r"\bno,?\s",
...
def main(src_path: str):
    ...
    print(
        f"p10={pct(0.1)}  p25={pct(0.25)}  p50={pct(0.5)}  p75={pct(0.75)}  p90={pct(0.9)}  p99={pct(0.99)}"
    )
    short = sum(1 for l in lens if l < 100)
    long_ = sum(1 for l in lens if l > 2000)
    print(
        f"<100 chars: {short}/{n} = {100 * short / n:.0f}%   >2000 chars: {long_}/{n} = {100 * long_ / n:.0f}%"
    )
