...
assert (_root / "config").is_dir() and (_root / ".git").exists(), (
    f"device-info.py: _root resolved to {_root}, which does not look like the "
    "repo root (missing config/ or .git) — check the parent-depth calculation "
    "above; .claude/hooks/ is one level deeper than scripts/."
)
...
def main():
    ...
    print(
        f"DEVICE: {device.get('name', '?')} | OS: {device.get('os', '?')} | "
