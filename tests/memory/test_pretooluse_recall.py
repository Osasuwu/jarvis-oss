...
_HOOK_PATH = Path(__file__).resolve().parent.parent.parent / "scripts" / "pretooluse-recall-hook.py"
...
class TestHelpers:
    ...
    @pytest.fixture(autouse=True)
    def _known_projects(self, monkeypatch):
        # KNOWN_PROJECTS derives from config/repos.conf at import time; pin a
        # fixture set so these tests don't depend on the clone's local config.
        monkeypatch.setattr(hook, "KNOWN_PROJECTS", {"jarvis", "redrobot"})
