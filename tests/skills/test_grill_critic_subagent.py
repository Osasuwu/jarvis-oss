...
class TestAC3CriticMdFixedSchema:
    ...
    def test_critic_md_specifies_max_3_risks_with_severity(self, critic_md: str):
        assert re.search(
            r"(<=|≤|at most|max|maximum|up to)\s*3\s*risks?", critic_md, re.IGNORECASE
    ...
    def test_critic_md_specifies_max_3_alternatives(self, critic_md: str):
        assert re.search(
            r"(<=|≤|at most|max|maximum|up to)\s*3\s*(unmentioned\s+)?alternatives?",
            critic_md,
            re.IGNORECASE,
    ...
    def test_critic_md_specifies_one_challenged_assumption(self, critic_md: str):
        ...
        assert re.search(
            r"1\s+challenged\s+assumption|one\s+challenged\s+assumption", critic_md, re.IGNORECASE
    ...
    def test_critic_md_forbids_freeform_prose(self, critic_md: str):
        ...
        assert re.search(
            r"no prose|not\s+prose|do not.*prose|don'?t.*prose|fixed (schema|format)",
            critic_md,
            re.IGNORECASE,
...
class TestAC4LoopbackBlocksAcLock:
    ...
    def test_skill_md_states_disposition_blocks_ac_lock(self, skill_md: str):
        ...
        assert re.search(
            r"block.*AC[- ]lock|AC[- ]lock.*block|cannot.*lock.*AC|AC[- ]lock.*until",
            skill_md,
            re.IGNORECASE,
    ...
    def test_skill_md_states_per_item_not_bulk(self, skill_md: str):
        ...
        assert re.search(
            r"per[- ]item|per[- ]finding|each (risk|alternative|finding|item)",
            skill_md,
            re.IGNORECASE,
