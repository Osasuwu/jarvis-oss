...
class TestResearchSkillStructure:
    ...
    def setup_class(cls):
        ...
        assert cls.skill_content is not None, "Could not find /research SKILL.md"
    ...
    def test_four_channel_protocol_section_exists(self):
        ...
        assert has_channel_section, "SKILL.md must have 4-channel protocol section"
    ...
    def test_channel_1_description_and_examples(self):
        ...
        channel_1_block = re.search(
            r"##\s*Channel\s*1.*?(?=##\s*Channel|$)", self.skill_content, re.IGNORECASE | re.DOTALL
        )
        assert channel_1_block is not None, "Channel 1 (Users) section must exist"
    ...
    def test_channel_2_description_and_examples(self):
        ...
        channel_2_block = re.search(
            r"##\s*Channel\s*2.*?(?=##\s*Channel|$)", self.skill_content, re.IGNORECASE | re.DOTALL
        )
        assert channel_2_block is not None, "Channel 2 (Specialists) section must exist"
    ...
    def test_channel_3_description_and_examples(self):
        ...
        channel_3_block = re.search(
            r"##\s*Channel\s*3.*?(?=##\s*Channel|$)", self.skill_content, re.IGNORECASE | re.DOTALL
        )
        assert channel_3_block is not None, "Channel 3 (Data) section must exist"
    ...
    def test_channel_4_description_and_examples(self):
        ...
        channel_4_block = re.search(
            r"##\s*Channel\s*4.*?(?=##\s*Channel|$)", self.skill_content, re.IGNORECASE | re.DOTALL
        )
        assert channel_4_block is not None, "Channel 4 (Adversarial) section must exist"

    def test_scope_note_mentions_grandfathered_memories(self):
        """AC: SKILL.md contains Scope noting grandfathered memories."""
        has_scope = bool(re.search(r"Scope|grandfathered", self.skill_content, re.IGNORECASE))
        assert has_scope, "SKILL.md must include Scope section"

    def test_mandatory_coverage_enforcement_note(self):
        """AC: SKILL.md states all 4 channels are mandatory."""
        assert re.search(r"mandatory|all.*4.*channel", self.skill_content, re.IGNORECASE), (
