...
class TestPhase1SessionParameterGate:
    ...
    def test_phase1_asks_cadence_with_frontier_default_and_single_question_fallback(
        self, phase1_section: str
    ):
        assert re.search(r"cadence", phase1_section, re.IGNORECASE), "Phase 1 must ask for cadence"
...
class TestPhase2FrontierRounds:
    ...
    def test_questions_asked_together_in_numbered_round(self, phase2_section: str):
        assert re.search(
            r"numbered round|together in one round|one numbered round",
            phase2_section,
            re.IGNORECASE,
        ), (
    ...
    def test_questions_tagged_design_forming_or_refining(self, phase2_section: str):
        assert "design-forming" in phase2_section, "Phase 2 must tag questions 'design-forming'"
        assert "refining" in phase2_section, "Phase 2 must tag questions 'refining'"
    ...
    def test_thin_design_forming_answer_triggers_followup(self, phase2_section: str):
        assert re.search(
            r"thin.*design-forming|design-forming.*thin", phase2_section, re.IGNORECASE
    ...
    def test_bare_agreement_not_a_trigger(self, phase2_section: str):
        ...
        assert re.search(
            r"not.*(itself\s+)?a\s+trigger|never.*trigger", phase2_section, re.IGNORECASE
    ...
    def test_round_summary_two_line_ceiling(self, phase2_section: str):
        ...
        assert re.search(r"settl", phase2_section, re.IGNORECASE) and re.search(
            r"next frontier", phase2_section, re.IGNORECASE
    ...
    def test_answer_conflict_reopens_earlier_question(self, phase2_section: str):
        ...
        assert re.search(r"glossary", phase2_section, re.IGNORECASE) and re.search(
            r"code", phase2_section, re.IGNORECASE
